...
void test_zero(void) {
    ...
    bool success = jstruct_parse(&u32_value, test_value, &out, &result);
...
void test_max_value(void) {
    char const* const test_value = "4294967295";
    uint32_t out = 0;
...
void test_mid_value(void) {
    ...
    TEST_ASSERT_TRUE(success);
    TEST_ASSERT_EQUAL(JSTRUCT_PARSE_RESULT_TYPE_PASS, result.type);
    TEST_ASSERT_EQUAL_UINT32(2147483648U, out);
}

void test_out_of_bounds_positive(void) {
    char const* const test_value = "4294967296";
    uint32_t out = 0;
...
void test_expects_integer(void) {
    ...
    for (size_t i = 0; i < num_test_values; i++) {
        ...
        jstruct_parse_result_t result = {0};
        bool const success = jstruct_parse(&u32_value, test_values[i], &out, &result);
