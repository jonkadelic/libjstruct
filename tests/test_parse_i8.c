...
static jstruct_value_t const i8_value = {
    .type = JSTRUCT_VALUE_TYPE_I8,
};
...
void test_zero(void) {
    ...
    bool success = jstruct_parse(&i8_value, test_value, &out, &result);

    TEST_ASSERT_TRUE(success);
    TEST_ASSERT_EQUAL(JSTRUCT_PARSE_RESULT_TYPE_PASS, result.type);
    TEST_ASSERT_EQUAL_INT8(0, out);
}

void test_max_value(void) {
    char const* const test_value = "127";
    int8_t out = 0;
    ...
    TEST_ASSERT_TRUE(success);
    TEST_ASSERT_EQUAL(JSTRUCT_PARSE_RESULT_TYPE_PASS, result.type);
    TEST_ASSERT_EQUAL_INT8(127, out);
}
...
void test_min_value(void) {
    ...
    TEST_ASSERT_TRUE(success);
    TEST_ASSERT_EQUAL(JSTRUCT_PARSE_RESULT_TYPE_PASS, result.type);
    TEST_ASSERT_EQUAL_INT8(-128, out);
}

void test_negative_value(void) {
    char const* const test_value = "-50";
    int8_t out = 0;
...
void test_out_of_bounds_negative(void) {
    char const* const test_value = "-129";
    int8_t out = 0;
...
void test_expects_integer(void) {
    ...
    for (size_t i = 0; i < num_test_values; i++) {
        ...
        jstruct_parse_result_t result = {0};
        bool const success = jstruct_parse(&i8_value, test_values[i], &out, &result);
